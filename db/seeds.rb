...
(1..5).each do |n|
  Question.create!(:number => n, :title => "Choose one word or phrase that best completes the sentence.", :description => "Your prompt _______ to our proposal would be highly appreciated.", :choice1 => "respond", :choice2 => "response", :choice3 => "to respond", :choice4 => "responded", :answer => 2, :explanation => "ビジネス・レターでよく使用される文です。Your の後ろには名詞があることが予想されますが、prompt の品詞を見極めることがまず大切です。これが「迅速な」の意味の形容詞であることが分かれば、その後ろに名詞がこなければいけないことが明らかになります。")
  Question.create!(:number => n, :title => "Choose one word or phrase that best completes the sentence.", :description => "Let them _______ as they like. They'll never listen to us.", :choice1 => "do", :choice2 => "to do", :choice3 => "to be doing", :choice4 => "to have done", :answer => 1, :explanation => "「…させてあげる」の意味の使役動詞 let は、let ＋目的語＋原形動詞の形をとります。")
  Question.create!(:number => n, :title => "Choose one word or phrase that best completes the sentence.", :description => "I was just about to leave home, _______ the phone rang.
", :choice1 => "that", :choice2 => "why", :choice3 => "when", :choice4 => "at that time", :answer => 3, :explanation => "【着眼点】選択肢のうちで、接続詞機能を有し、かつコンマを介して2つの節を連結できるのは非制限用法をもつ関係副詞のwhenのみである。なお、この場合のwhenは前の節全体を先行詞とする。be about to ～ は「まさに［今にも］～しようとしている」の意。「ちょうど家を出ようとしていたそのときに、電話が鳴った。」")
